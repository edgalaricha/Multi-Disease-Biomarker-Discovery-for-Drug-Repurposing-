...
# Count Triangles
# Function to find all unique triangles
find_triangles <- function(graph) {
  tri <- triangles(graph)
  
  # `triangles()` returns a vector, so we convert it into groups of 3
  tri_matrix <- matrix(tri, ncol = 3, byrow = TRUE)
  
  # Convert vertex IDs to gene names
  tri_names <- apply(tri_matrix, 1, function(x) {
    sort(V(graph)$name[x])
  })
  
  tri_df <- as.data.frame(t(tri_names))
  colnames(tri_df) <- c("Gene1", "Gene2", "Gene3")
  
  tri_df <- distinct(tri_df)
  
  return(tri_df)
}

# Find triangles in each graph
...
# Count how many triangles each gene participates in
count_triangles_per_gene <- function(triangle_df) {
  triangle_df %>%
    pivot_longer(
      cols = everything(),
      names_to = "triangle_position",
      values_to = "gene"
    ) %>%
    count(gene, name = "triangle_count")
}


astro_counts <- count_triangles_per_gene(astro_triangles)
gbm_counts   <- count_triangles_per_gene(gbm_triangles)
oligo_counts <- count_triangles_per_gene(oligo_triangles)

# Build unified set of genes
...
triangles_count <- tibble(gene = all_genes) %>%
  left_join(astro_counts, by = "gene") %>%
  dplyr::rename(Astro = triangle_count) %>%
  left_join(gbm_counts, by = "gene") %>%
  dplyr::rename(GBM = triangle_count) %>%
  left_join(oligo_counts, by = "gene") %>%
  dplyr::rename(Oligo = triangle_count) %>%
  mutate(
    Astro = replace_na(Astro, 0),
    GBM = replace_na(GBM, 0),
    Oligo = replace_na(Oligo, 0)
  ) %>% 
...
saveRDS(triangles_count, "outputs/triangles_count.rds")
